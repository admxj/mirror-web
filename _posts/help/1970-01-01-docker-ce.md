...
<code class="language-bash" id="deb-amd64-content">sudo add-apt-repository \
   "deb [arch=amd64] https://mirrors.admxj.cn/docker-ce/linux/{{deb_release}} \
   $(lsb_release -cs) \
   stable"</code>
</pre>
{% endraw %}
...
<p></p>
<pre>
<code id="deb-arm-content">echo "deb [arch=armhf] https://mirrors.admxj.cn/docker-ce/linux/{{deb_release}} \
...
```bash
sudo sed -i 's+download.docker.com+mirrors.admxj.cn/docker-ce+' /etc/yum.repos.d/docker-ce.repo
```
